...
pub mod dialplan_service_server {
    ...
    #[derive(Debug)]
    pub struct DialplanServiceServer<T: DialplanService> {
        inner: Arc<T>,
    ...
    impl<T: DialplanService> DialplanServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: DialplanService> Clone for DialplanServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: DialplanService> tonic::server::NamedService for DialplanServiceServer<T> {
        const NAME: &'static str = "sentiric.dialplan.v1.DialplanService";
    }
}
