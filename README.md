...
Yeni bir sürüm etiketlendiğinde (örn: `v1.8.12`), release pipeline'ı paketleri otomatik olarak yayınlar. Diğer servisler bu paketleri bağımlılık olarak kullanmalıdır.

### Go
`go.mod` dosyanıza ekleyin:
```go
require (
    github.com/sentiric/sentiric-contracts v1.8.12
)
```
...
3.  Yeni paket versiyonları yayınlamak için yeni bir Git etiketi oluşturun (örn: `git tag v1.8.12` ve `git push origin v1.8.12`). Release pipeline'ı gerisini halleder.
...
git push --delete origin v1.8.12
```
